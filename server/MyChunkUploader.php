 ...
 *
 * @id      : MyChunkUploader.php | Fri Mar 18 17:18:30 2016 +0100 | eugenmihailescu <eugenmihailescux@gmail.com> $
 */
...
class MyChunkUploader {
    ...
    private function _copy_file( $input_file, $output_file, $sufix = '', $write_mode = 'wb' ) {
        ...
        ( false !== $fr ) || $this->_set_error( "Unable to open $input_file", "3.4$sufix" );

        $fw = @fopen( $output_file, $write_mode );
        ( false !== $fw ) || $this->_set_error( "Unable to open $output_file", "3.5$sufix" );
    ...
    private function _merge_files() {
        ...
        $target_file_name = $this->_tmp_dir . "/" . $this->_filename;

        if (file_exists($target_file_name)) {
            if (!unlink($target_file_name)) {
                $this->_set_error( "File chunk prevents from uploading: $target_file_name", "3.4final" );
            }
        }

        // concatenate all chunk files into a single final file
        foreach ( $files as $chunk_filename ) {
            $this->_copy_file( $chunk_filename, $target_file_name, 'final', 'ab' );
            unlink( $chunk_filename );
        }
        ...
        if ( is_callable( $this->on_done ) && count( $files ) ) {
            try {
                $this->_filename = call_user_func( $this->on_done, $this->_filename, $this->_tmp_dir, $target_file_name);
            } catch ( \Exception $e ) {
    ...
    public function run() {
        ...
        if (file_exists($tmp_filename)) {
            if (!unlink($tmp_filename)) {
                $this->_set_error( "File chunk prevents from uploading: $tmp_filename", "3.4chunk" );
            }
        }
